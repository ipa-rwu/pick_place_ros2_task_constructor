...
PickPlaceServer::PickPlaceServer(const rclcpp::NodeOptions& node_options)
    : node_{std::make_shared<rclcpp::Node>("tpick_place_server_node",
                                           node_options)} {}

PickPlaceServer::PickPlaceServer(const std::string& node_name,
  ...
  service_ = node_->create_service<pick_place_msgs::srv::PickPlace>(
      "pick_place_service",
      std::bind(&PickPlaceServer::pickPlaceCallback, this, _1, _2, _3),
...
void PickPlaceServer::pickPlaceCallback(
  ...
  RCLCPP_INFO(LOGGER, "object_frame_id: %s",
              object_pose.header.frame_id.c_str());

  RCLCPP_INFO(LOGGER, "place_frame_id: %s",
              pick_place_parameters_.object_frame_id.c_str());

  moveit::planning_interface::PlanningSceneInterface psi;
  // get pose from param
  psi.applyCollisionObject(
      createObject(object_name, object_pose.header.frame_id,
